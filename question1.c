...
void read_int(int *dest, const char *prompt) {
    ...
    // This is safely uninitialised as it is not read until after it is written to.
    char buffer[12];
    // The return code from the call to `sscanf()` used to parse an numeric string.
    int result;
...
int main(void) {
    ...
    printf("Please enter the number of array elements you would like to input.\n\n");
    ...
    elems = malloc(sizeof(int) * num_elems);
